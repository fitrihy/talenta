...
<div class="kt-portlet kt-portlet--mobile">
    ...
    <div class="kt-portlet__body form">
    	<form class="kt-form kt-form--label-right" method="post" id="form-datapokok" role="form" enctype="multipart/form-data" action="/administrasi/bumn/{{ $suratkeputusan->id }}">
			@csrf
			{{ method_field('PUT') }}
			<div class="kt-portlet__body">
                <input type="hidden" name="id_surat_keputusan" id="id_surat_keputusan" readonly="readonly" value="{{$suratkeputusan->id}}" />
				...
				<div class="form-group row">
                    <div class="col-lg-6">
                        ...
                        <select class="form-control kt-select2" required="required" name="id_perusahaan">
                            ...
                            <option value="{{ $perusahaan->id }}" <?=old('id_perusahaan') == $perusahaan->id || $suratkeputusan->id_perusahaan == $perusahaan->id? 'selected="selected"':''?>>{{ $perusahaan->nama_lengkap }}</option>
                            @endforeach
                        </select>
                    </div>
                    <div class="col-lg-6">
                        <label>Grup Jabatan</label>
                        <select class="form-control kt-select2" id="grup_jabatan_id" required="required" name="grup_jabatan_id" onchange="return onNamaJenisSK(this.value)">
                            <option value=""></option>
                            @foreach($grupjabatans as $grupjabatan)

                            <option value="{{ $grupjabatan->id }}" <?=old('grup_jabatan_id') == $grupjabatan->id || $suratkeputusan->id_grup_jabatan == $grupjabatan->id? 'selected="selected"':''?>>{{ $grupjabatan->nama }}</option>
                ...
                <div class="form-group row">
                    <div class="col-lg-4">
                        ...
                        <input type="text" required="required" class="form-control kt-shape-bg-color-1" name="nomor_sk" id="nomor_sk" value="{{ $suratkeputusan->nomor }}" />
                        <span class="form-text text-muted">Masukan Nomor SK</span>
                    </div>
                    <div class="col-lg-4">
                        <label>Tanggal SK<span style="color: red">*</span></label>
                        <input type="text" id="tgl_sk" name="tgl_sk" required="required" class="form-control kt-shape-bg-color-1 input-sm cls-datepicker" value="{{ $tanggal_sk }}" />
                        <span class="form-text text-muted">Masukan Tanggal SK</span>
                    </div>
                    <div class="col-lg-4">
                        <label>Tanggal Serah Terima SK<span style="color: red">*</span></label>
                        <input type="text" id="tanggal_serah_terima" name="tanggal_serah_terima" placeholder="Masukan Tanggal SK" required="required" class="form-control kt-shape-bg-color-1 input-sm cls-datepicker" value="{{ $tanggal_serah_terima }}" />
                ...
                <div class="form-group row">
                    <div class="col-lg-5">
                        ...
                        <select class="form-control multi-select" id="jenis_sk_id" name="jenis_sk_id[]" multiple="multiple">
                            ...
                            @php
                               $select = !empty(old('jenis_sk_id')) && in_array($jenissk->id, old('jenis_sk_id'))? 'selected="selected"' : (in_array($jenissk->id, $rinsianhassk)? 'selected="selected"' : '')
                            @endphp
                ...
                <div class="form-group row">
                    <div class="col-lg-4">
                        <label>File SK</label>
                            <input type="file" name="file_name" class="form-control kt-shape-bg-color-1" id="file_name" value="{{$file_sk}}">
                            <span class="form-text text-muted">Upload File PDF</span>
                    </div>
                    @if(!empty($file_sk))
                    <div class="col-lg-3">
                        <label>File PDF</label><br>
                            <a href="/uploads/suratkeputusanfile/{{$file_sk}}" target="_blank">{{ $suratkeputusan->nomor }}</a>
                ...
                <div class="form-group row">
                    <div class="col-lg-12">
                        ...
                        <textarea class="form-control kt-shape-bg-color-1" id="keterangan" name="keterangan" rows="5" cols="5">{{ $keterangan }}</textarea>
                    </div>
                </div>
			</div>
...
function onReadEvent(){
  onLoadMultiSelect();
  /*var id_grup_jabatan = $('input[name=grup_jabatan_id]').val();
  onNamaJenisSK(id_grup_jabatan);*/
}
