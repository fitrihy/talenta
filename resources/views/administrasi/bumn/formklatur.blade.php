<form class="kt-form kt-form--label-right" method="POST" id="form-klatur">
	@csrf
		...
		<div class="form-group row">
			...
			<div class="col-lg-6">
				...
				<select class="form-control kt-select2" name="id_struktur_organ">
                    ...
                    @php
            	      $select = !empty(old('id_struktur_organ')) && in_array($jabatan->id, old('id_struktur_organ'))? 'selected="selected"' : ($actionform == 'update' && ($jabatan->id==$skklatur->id_struktur_organ)? 'selected="selected"' : '')
            	   @endphp
			...
			<div class="col-lg-6">
				...
				<input type="text" class="form-control" name="nomenklatur_baru" id="nomenklatur_baru" value="{{!empty(old('nomenklatur_baru'))? old('nomenklatur_baru') : ($actionform == 'update' && $skklatur->nomenklatur_baru != ''? $skklatur->nomenklatur_baru : old('nomenklatur_baru'))}}" />
			</div>
		</div>
	</div>
...
<script type="text/javascript" src="{{asset('js/administrasi/bumn/formklatur.js')}}"></script>